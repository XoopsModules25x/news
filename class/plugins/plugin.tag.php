...
function news_tag_synchronization($mid)
{
    ...
    if ($link_handler->mysql_major_version() >= 4):
        $sql = "	DELETE FROM {$link_handler->table}" . "	WHERE " . "		tag_modid = {$mid}" . "		AND " . "		( tag_itemid NOT IN " . "			( SELECT DISTINCT {$item_handler_keyName} "
            . "				FROM {$item_handler_table} " . "				WHERE $where" . "			) " . "		)";
    else:
        $sql
            =
            "	DELETE {$link_handler->table} FROM {$link_handler->table}" . "	LEFT JOIN {$item_handler_table} AS aa ON {$link_handler->table}.tag_itemid = aa.{$item_handler_keyName} " . "	WHERE "
            . "		tag_modid = {$mid}" . "		AND " . "		( aa.{$item_handler_keyName} IS NULL" . "			OR " . "       (aa.published > 0 AND aa.published <= " . time()
            . ") AND (aa.expired = 0 OR aa.expired > " . time() . ')' . "		)";
