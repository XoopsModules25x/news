...
if (!empty($_POST['submit'])) { // The form was submited
    ...
    if ($ratinguser != 0) {
        ...
        $result    = $xoopsDB->query(
            'SELECT COUNT(*) FROM ' . $xoopsDB->prefix('news_stories_votedata') . " WHERE storyid=$storyid AND ratinguser=0 AND ratinghostname = '$ip'  AND ratingtimestamp > $yesterday"
        );
    ...
    $datetime = time();
    $sql = sprintf(
        "INSERT INTO %s (ratingid, storyid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (%u, %u, %u, %u, '%s', %u)",
